...
in one place.
For this we are currently using the Devise gem for users (authentication, sign ups, etc.), the acts_as_votable gem
for the post voting system and the Simple Form gem for the comment system.
During development we are using the default WEBrick web server, just so we can get things up and running, without
initializing an apache server which we will be configuring later in development.
:+1:
